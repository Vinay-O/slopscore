...
function printRules() {
  ...
  out(`slopscore ships ${all.length} deterministic detectors. The full 281-pattern catalog`);
...
function printExplain(arg) {
  ...
  if (start === -1) { err(`slopscore: no catalog entry ${id} (ids run 001–281). Try: slopscore protocol`); process.exit(2); }
...
USAGE
  ...
  slopscore protocol            print the full 281-pattern protocol (pipe to your agent)
